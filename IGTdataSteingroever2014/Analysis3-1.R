...
LL <- function(subID){
  function(pp){
    ...
    a <- pp[2]
    c <- pp[3]
    valw <- as.numeric(valscale*w*win_data[subID,])+as.numeric(valscale*(1-w)*lose_data[subID,])
    EE <- NULL
    for(j in 1:4){
      E <- NULL
      for (t in 1:100) {
        ...
        if (t==1) {
          ...
          # 2回目以降の更新
          if (choice_data[subID,t]==j) {
            EV <- expected_valence(a,t,valw,E)
            E <- c(E,EV)
          }else{
            E <- c(E,E[t-1])
          }
...
system.time(
  result <- optim(par=c(0.5,0.5,0.01),fn=LL(1),method = "L-BFGS-B",lower = c(0,0,-2),upper = c(1,1,2),control=list(fnscale=-1))
)

Esti <- matrix(0, 162, 3)
for(i in 1:162){
  mle <- optim(par=c(0.5,0.5,0.01), fn=LL(i), lower=c(0.001,0.001,-1), upper=c(0.999,0.999,1), method="L-BFGS-B", control=list(fnscale=-1))
  Esti[i,] <- mle$par
}
